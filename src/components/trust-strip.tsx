...
export function TrustStrip({ stats, className, variant = "stats" }: TrustStripProps) {
  return (
    <div
      className={cn(
        "overflow-hidden rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface)] shadow-[var(--elevation-1)]",
        className,
      )}
    >
        ...
        <div className="grid grid-cols-3 divide-x divide-[var(--line)]">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="flex flex-col items-center justify-center gap-1 px-2 py-3 sm:flex-row sm:gap-2.5 sm:px-4 sm:py-3.5"
              >
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--accent-soft)] text-[var(--accent)]">
                  <Icon className="h-3.5 w-3.5" />
                </span>
                <div className="text-center sm:text-left">
                  <p className="text-base font-bold tabular-nums tracking-[-0.03em] text-[var(--foreground)] sm:text-lg">
                    {stat.value}
                  </p>
      ...
      {variant === "stats" ? (
        <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-1 border-t border-[var(--line)] px-3 py-2 text-[11px]">
