...
/** Native Vendor blog management activity. Lists blogs for the logged-in vendor with delete support. */
class VendorBlogsActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val content = scaffold("My Blogs", withBack = true)
        content.block(text("Your Published Blogs", 24f, ink, Typeface.BOLD), 4)
        content.block(text("Review, read, or delete blogs published for your products.", 14f, muted), 12)
        body = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
        content.block(body, 0)
        load(session.vendorSlug.orEmpty())
    }

    private fun load(vendorSlug: String) {
        body.removeAllViews()
        ...
        executor.execute {
            ...
            runOnUiThread {
                ...
                for (i in 0 until blogs.length()) {
                    ...
                    if (b.optString("vendorSlug") == vendorSlug) {
                        body.addView(blogCard(b).also { animateIn(it, count++) })
                    }
                }
                if (count == 0) {
                    body.addView(errorCard("You haven't published any blogs yet."))
                }
            }
        }
    }
